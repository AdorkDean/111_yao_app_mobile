 ...
 *
 */
public class AddReturnLogisticsService extends BaseURIService {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(StringUtils.hasText(sessionid)){
			...
			try {
				...
				if(0<userId){
					String rid = request.getParameter("retuId");//售后单ID
					String expressNo = request.getParameter("expressNo");//物流单号
					String logisticsCompany = request.getParameter("logisticsCompany");//物流公司
					TReturn bean = new TReturn();
					if(null!=rid&&!"".equals(rid.trim())){
						bean.setId(Long.parseLong(rid));
						bean.setExpressDelivery(expressNo);
						bean.setExpressCompany(logisticsCompany);
						((TReturnManager)getSpringBean("treturnmanager")).updateByPrimaryKeySelective(bean);
						statusCode = "1";
					}else {
						statusCode = "0";
					}
