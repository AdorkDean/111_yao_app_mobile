 ...
 *
 */
public class AddCouponService extends BaseURIService {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(StringUtils.hasText(sessionid)){
			...
			try {
				...
				if(0<userId){
					String code = request.getParameter("code");//优惠码
					TCouponCard bean = new TCouponCard();
					if(null!=code&&!"".equals(code.trim())){
						bean.setCardNo(code);
						bean.setMemberId(userId);
						int scode = ((TCouponCardManager)getSpringBean("tcouponcardmanager")).updateByPrimaryKeySelective(bean);
						statusCode = scode+"";
					}else {
						statusCode = "-1";
					}
