 ...
 *
 */
public class DelUserAreaAreaService extends BaseURIService {
		...
		if(null != sessionId && !("").equals(sessionId) && null != areaId && !("").equals(areaId)){
			...
			try {
				...
				if(null != userId && userId != -1){
					...
					TMemberReceiverExample tmre = new TMemberReceiverExample();
					tmre.createCriteria().andMemberIdEqualTo(userId).andIdEqualTo(new Long(areaId));
					int i = tmemberreceivermanager.deleteByExample(tmre);
					if(i>0){
