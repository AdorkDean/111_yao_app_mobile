 ...
 *
 */
public class GetHomePageInfoService2 extends BaseURIService {
	...
	String str = null;
		try {
			...
			try {
				str = (String) MemCached.getmcc().get("homeDatahd"+client_type);
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			if(null==str||"".equals(str)){
				...
				try {
					MemCached.getmcc().add("homeDatahd"+client_type,str,new Date(1000*60*30));
				}catch (Exception e) {
					e.printStackTrace();
				}
			}
...
//		String homePageConfigId=InfoUtil.getInstance().getInfo("config", "homePageConfigId");
		...
		String homePageConfigId = sys.getKeys("homePageConfigId");
