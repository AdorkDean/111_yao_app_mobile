 ...
 *
 */
public class GetHomePageActivityService2 extends BaseURIService {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if("1".equals(pageNo)){
				try {
					str = "";//(String) MemCached.getmcc().get("homeDataxx"+client_type);
				} catch (Exception e1) {
					e1.printStackTrace();
				}
			}
			if(null==str||"".equals(str)){
				...
				if("1".equals(pageNo)){
					try{
						//MemCached.getmcc().add("homeDataxx"+client_type,str,new Date(1000*60*30));
					}catch (Exception e) {
						e.printStackTrace();
					}
				}
...
//		String cid =InfoUtil.getInstance().getInfo("config", "homePageActivityConfigId");
		...
		pw = om.selectForPageByMap(map, "c_position.selectPositionByPageCount2", "c_position.selectPositionByPage2", rs.getP_curPage(), rs.getP_pageSize());
		return pw.getResult();
	}
}
