...
public class CreateOrderService extends BaseURIService{
	private static final long serialVersionUID = -8924997236120193694L;
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String isInvoice = request.getParameter("isInvoice");//是否开具发票   1 不开   2 开
							...
							if("".equals(statusCode)){
								...
								if(statusCode.equals("")){
									...
									obj =tordermanager.createOrder(orderVO, memberId, paymentmethodId, receiverId, 
											NumberUtil.string2Long(couponCodeIds), null, invoiceTitle, paymentPluginId,source,true,request);
									if(orderVO.getAmountPayable().compareTo(new BigDecimal(0)) <= 0){//待支付金额等于0
										payFlag="2";
									}
									statusCode="1";
								    long end =System.currentTimeMillis();
								    log.info("V2创建订单耗时==================》"+String.valueOf(end-start)+"毫秒");
		...
		if(StringUtils.hasText(paymentPluginId)&&"1".equals(statusCode)&&"1".equals(payFlag)){
			...
			log.info("请修改后重新下单！您购买的"+errorMessage.substring(0, errorMessage.length()-1)+"是处方药，目前处方药仅支持北京部分地区以货到付款的方式购买。" );
			map.put("errorMessage", "请修改后重新下单！您购买的"+errorMessage.substring(0, errorMessage.length()-1)+"是处方药，目前处方药仅支持北京部分地区以货到付款的方式购买。" );
		}
